...
<!--PATTERN -->
	...
	<!--RULE -->
<xsl:template match="xs:MPD[@c:mpdClassCode = ('iepd', 'eiem')]"
                 priority="1000"
                 mode="M2">

		<!--ASSERT -->
<xsl:choose>
         <xsl:when test="count(c:BaseSchemaDocSet) = 1"/>
         <xsl:otherwise>
            ...
            <xsl:text xmlns:sch="http://www.ascc.net/xml/schematron">:assert:</xsl:text>An MPD that is an IEPD or EIEM MUST have exactly one base schema document set.<xsl:value-of xmlns:sch="http://www.ascc.net/xml/schematron" select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*" mode="M2"/>
   </xsl:template>
