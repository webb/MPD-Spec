...
<section id="scope-iepd"><title>Information Exchange Package Documentation</title>
	...
	<ul>
		<li><p>An XML document with a NIEM-defined XML document element.</p></li>
		<li><p>An XML document with a NIEM-defined payload element wrapped inside a non-NIEM envelope element (for example, SOAP, <ref idref="LEXS"/> (LEXS), Trusted Data Format (TDF), or an OGC Web Service document element).</p></li>
		<li><p>Multiple NIEM-defined payloads packaged together in a single document.</p></li>
		<li><p>A non-NIEM format defined with no NIEM content at all (for example, a data exchange specified by a GML application schema).
		</p></li>
		<li><p>A non-NIEM format in which optional NIEM elements do not occur (for example, optional hospital information, provided as a NIEM element).
		</p></li>
	</ul>
...
<section id="Character-Case-Sensitivity"><title>Character Case Sensitivity</title>
...
<section id="Schema-Namespace-Correspondence-in-NIEM">
	...
	<p>To simplify automatic schema processing and reduce the potential for confusion and error, <ref idref="NIEM-NDR"/> principles state that each NIEM-conformant namespace SHOULD be defined by exactly one reference or extension schema document.  To support this concept, the <ref idref="NIEM-NDR"/> disallows the use of <qName>xs:include</qName>, and mandates the use of the <code>xs:schema/@targetNamespace</code> attribute in NIEM-conformant schema documents.</p>
...
<section id="Namespaces"><title>Namespaces Used in this Specification</title>

	<p>The following namespaces are referenced and used in this specification:</p>

	<figure>
	  <title>Namespaces Used</title>
		<pre>c           MACRO_namespace_mpd_catalog
...
<section id="Harmonization"><title>Harmonization</title>

	<p>A key NIEM concept important to <termRef>harmonization</termRef> and used throughout this specification is <termRef>data component</termRef>.</p>
...
<section id="XML-Validation"><title>XML Validation</title>
...
<section id="Reference-Schema-Document"><title>Reference Schema Documents</title>
	...
	<definition term="reference schema document set">
		<p>A set of related <termRef term="reference schema document">reference schema documents</termRef>, such as a NIEM release.</p>
	</definition>

	<p>The <ref idref="NIEM-NDR"/> conformance rules for reference schema documents are generally stricter than those for other classes of NIEM-conformant schema documents.  For example, <termRef term="reference schema document">reference schema documents</termRef> are not allowed to employ particular XML Schema model groups such as <qName>xs:choice</qName> or <qName>xs:any</qName> that other schema documents may contain.</p>

	<p>NIEM reference schemas are very uniform in their structure.  As they are the primary definitions for <termRef term="data component">data components</termRef>, they do not need to restrict other data definitions, and so they are not allowed to use XML Schema<char name="rsquo"/>s complex type restriction mechanisms.</p>
...
<section id="Rules"><title>Rules</title>
          ...
          <p>Rules define specific constraints on artifacts or on the interpretation of artifacts.  The classes of artifacts are identified by <termRef term="conformance target">conformance targets</termRef> that are enumerated by this document in <ref idref="CT"/>.  Rules are normative.</p>
	  ...
	  <p>Each rule may apply to one or more <termRef term="conformance target">conformance targets</termRef>.  Each rule lists its applicable <termRef term="conformance target">conformance target(s)</termRef> encoded per <ref idref="ct-codes"/>.  The conformance targets for this specification are detailed in <ref idref="CT"/>.</p>
...
<section id="CT"><title>Conformance Targets</title>

	<p>This section introduces <termRef term="conformance target">conformance targets</termRef>, a concept fundamental to understanding the normative rules defined in this specification.  This section also defines and explains <termRef term="conformance target">conformance targets</termRef> used in this specification.</p>
...
<section id="CT-Terms"><title>Conformance Target Terminology</title>
	...
	<!--<p>It will also be useful to define <termRef>MPD class</termRef>.</p>

	<definition term="MPD class">
		<p>A <termRef>conformance target identifier</termRef> to which a given <termRef term="model package description">MPD</termRef> claims to conform.  The <termRef>MPD class</termRef> of an <termRef term="model package description">MPD</termRef> is established by <ref idref="r-mpdClassURIList"/>.</p>
	</definition>-->
	<p>It will also be useful to define <termRef>IEPD conformance target identifier</termRef>.</p>
	
	<definition term="IEPD conformance target identifier">
		<p>A <termRef>conformance target identifier</termRef> to which a given <termRef term="information exchange package documentation">IEPD</termRef> claims to conform.</p>
	</definition>
...
<section id="CT-IEPD"><title>IEPD Conformance Targets</title>
	...
	<definition term="ZIP file">
		<p>As defined by <ref idref="PKZIP"/>, which states that it defines:</p>
			<blockquote><p>... a cross-platform, interoperable file storage and transfer format ... used to aggregate, compress, and encrypt files into a single interoperable container.</p></blockquote>
	</definition>
	...
	<ul>
		...
		<li><p>Requires the following metadata:</p>
			<ul>
				...
				<li><p>The <termRef>conformance target identifier</termRef> <code>MACRO_IEPD_conformance_target_identifier</code> (See <ref idref="iepdConformanceTargetIdentifier"/>)</p></li>
			</ul>
		</li>
...
<!--	<definition term="model package description">
	  ...
	  <p>This term may be abbreviated <q>MPD</q>. Rules specifying this conformance target use the applicability code <q>WF-IEPD</q>.</p>
        ...
        <p>The schemas and other files within a <termRef>information exchange package documentation</termRef> are built on other specifications, including:</p>
	...
	<ruleSection>
		...
		<rule id="r-ct-iepd" applicability="WF-IEPD" class="Constraint">
			<p>An <termRef term="information exchange package documentation">IEPD</termRef> MUST have an <termRef>IEPD conformance target identifier</termRef> of <code>MACRO_IEPD_conformance_target_identifier</code> as a value of its <qName>c:iepdConformanceTargetIdentifierURIList</qName> attribute.</p>
		</rule>
	</ruleSection>
...
<section id="CT-IEPD"><title>IEPD Conformance Target</title>
	...
	<p>An IEPD also defines one or more data exchanges, one per <termRef>conformance target</termRef>.  Each data exchange occurs at runtime in the form of an <termRef>instance XML document</termRef>, which is an <termRef term="information exchange package">IEP</termRef> that conforms to the rules defined in the <qName>c:IEPConformanceTarget</qName> element.</p>

	<p>An <termRef term="information exchange package documentation">IEPD</termRef> contains a NIEM-conformant XML schema document set that may include portions of a NIEM core schema document (and supplements), portions of NIEM Domain schema documents (and updates), and enterprise-specific or IEPD-specific <termRef term="extension schema document">extension schema documents</termRef>.  The <termRef term="XML schema document">XML schema documents</termRef> contained in an <termRef term="information exchange package documentation">IEPD</termRef> work together to define one or more classes of <termRef term="instance XML document">instance XML documents</termRef> that consistently encapsulate data for meaningful information exchanges.  Any <termRef>instance XML document</termRef> that is valid for the <termRef>XML schema document</termRef> set and that satisfies the conditions of the <termRef>IEP conformance target</termRef> is a member of that <termRef>IEP conformance target</termRef> class (or IEP Class).</p>

	<p><termRef term="XML schema document">XML schema documents</termRef> in an <termRef term="information exchange package documentation">IEPD</termRef> conform to the <ref idref="NIEM-NDR"/> and may use or extend data component definitions drawn from NIEM. An <termRef term="information exchange package documentation">IEPD</termRef> may also incorporate and use XML schema documents from other standards that do not conform to NIEM.  (See <ref idref="NIEM-NDR"/> for details.)  An <termRef term="information exchange package documentation">IEPD</termRef> consists of a set of artifacts (XML schema documents, documentation, sample instance XML documents, etc.) that together define and describe one or more implementable data exchanges.  An <termRef term="information exchange package documentation">IEPD</termRef> should contain all materials necessary to:</p>
	<ul>
		<li><p>Understand information exchange context, content, semantics, and structure.</p></li>
		<li><p>Create and validate XML documents defined by the <termRef term="information exchange package documentation">IEPD</termRef>, and used for information exchanges.</p></li>
		<li><p>Identify the lineage of the <termRef term="information exchange package documentation">IEPD</termRef> itself and optionally its artifacts.</p></li>
	</ul>
	...
	<p>How to declare validity constraints for one or more IEP classes within an <termRef term="information exchange package documentation">IEPD</termRef> will be covered in more depth in <ref idref="Defining-Information-Exchange-Packages"/>.</p>
...
<section id="CT-IEP"><title>IEP Conformance Targets</title>
...
<section id="CT-Codes"><title>Rule Applicability Codes for Conformance Targets</title>

	<p>The table below lists the codes that represent standard <termRef term="conformance target">conformance targets</termRef> used in this specification and that appear in the applicability attribute for each rule.</p>

	<table id="ct-codes"><title>Rule Applicability Codes</title>
		<theadr>
		...
		<tr>
		    ...
		    <td><conformance-target id="IEP">IEP</conformance-target></td>
		</tr><tr>
...
<section id="Reference-Schemas"><title>Reference Schema Documents</title>
	...
	<p>As authoritative definitions, NIEM reference schema document sets satisfy more rigorous documentation requirements.  The <ref idref="NIEM-NDR"/> requires that each type definition, and element and attribute declaration in a reference schema document contain an <qName>xs:annotation</qName> element that defines its semantic meaning.</p>

	<p>Typically reference schema documents contain <termRef term="data component">data components</termRef> with the most relaxed cardinality (zero to unbounded).  However, this is not an absolute requirement.  If necessary, cardinality in reference schema documents may be constrained to model reality.  For example, in NIEM 3.0 a <qName>nc:Location2DGeospatialCoordinateType</qName> contains both a <qName>nc:GeographicCoordinateLatitude</qName> element and a <qName>nc:GeographicCoordinateLongitude</qName> element.  Each of these elements has cardinality <code>minOccurs="1"</code> and <code>maxOccurs="1"</code>.  Any other cardinality for these elements has no meaning.  On the other hand, one might claim that NIEM should constrain <qName>nc:PersonType</qName> to a single occurrence of the element <qName>nc:PersonBirthDate</qName>.  Every person has one and only one birth date.  Unfortunately, also in reality, criminal persons often present multiple identities with multiple birth dates; and so the capability to represent such is an important data requirement for NIEM.</p>

</section>



<section id="Subset-Schemas"><title>Subset Document Schemas</title>
...
<section id="Basic-Subset-Concepts">
	...
	<p>The primary purpose for a schema document subset is to reduce and constrain the scope and size of a full NIEM reference schema document set for use within an <termRef term="information exchange package documentation">IEPD</termRef>.  A schema document subset is derived from a reference schema document set (such as a NIEM release) by applying subset operations (See <ref idref="Constructing-Subsets"/>).  Also, note that employing a subset of a reference schema document set within an <termRef term="information exchange package documentation">IEPD</termRef> is optional; it is completely valid to reuse NIEM reference schema documents as-is within IEPDs.</p>
...
<ruleSection><title>Fundamental NIEM Subset Rule</title>
	...
	<p>A <termRef>schema document subset</termRef> is composed of <termRef term="XML schema document">XML schema documents</termRef>.  A <termRef>schema document subset</termRef> can essentially be a <termRef>reference schema document set</termRef> (i.e., a NIEM release) that has been modified by applying subset operations to support business requirements represented in an <termRef term="information exchange package documentation">IEPD</termRef>.  A subset derived from a reference schema document set may differ from that reference such that its content has been reduced and/or constrained.</p>
...
<section id="Constructing-Subsets"><title>Constructing a Schema Document Subset</title>

	<p>This section is non-normative.  Use the subset operations in this section with caution.</p>
	...
	<ol>
		...
		<li><p>Remove an <qName>xs:annotation</qName> and its children <qName>xs:documentation</qName> and <qName>xs:appinfo</qName>.</p></li>
		...
		<li><p>Remove an <qName>xs:element</qName> if its <code>@minOccurs="0"</code>.</p></li>
		<li><p>Remove an <qName>xs:complexType</qName> or <qName>xs:simpleType</qName> if not supporting an <qName>xs:element</qName> or <qName>xs:attribute</qName> declaration, or another <qName>xs:complexType</qName> or <qName>xs:simpleType</qName> definition.</p></li>
		<li><p>Remove an <qName>xs:attribute</qName> with <code>@use="optional"</code> from an <qName>xs:complexType</qName>.</p></li>
		...
		<li><p>Remove an <qName>xs:element</qName> declaration if it is not supporting an element use.</p></li>
		<li><p>Remove an <qName>xs:enumeration</qName> from an <qName>xs:simpleType</qName> as long as it is not the only remaining <qName>xs:enumeration</qName>.</p></li>
		...
		<li><p>Add or apply a constraining facet to an <qName>xs:simpleType</qName>.</p></li>
		<li><p>Remove an <qName>xs:import</qName> and its associated schema document if the schema document is not used within the document set.</p></li>
		<li><p>Change a concrete <qName>xs:element</qName> declaration to <code>@abstract="true"</code>.</p></li>
...
<section id="Extension-Schemas"><title>Extension Schema Documents</title>
	...
	<p>A developer who determines that NIEM is missing elements required for a given information exchange has several options for providing the missing elements.  Using rules and techniques defined in the <ref idref="NIEM-NDR"/>:</p>
	<ul>
		<li><p>Extend an existing NIEM <termRef>data component</termRef> (if possible).</p></li>
	...
	<p>A NIEM extension schema document may contain <termRef term="data component">data components</termRef> built from any of the options above.  Employment of extension schema documents in an <termRef term="information exchange package documentation">IEPD</termRef> is entirely optional.</p>

	<p>Multiple extension schema documents are allowed in a single <termRef term="information exchange package documentation">IEPD</termRef>.  Developers will likely want to reuse many of their extension schema documents in other IEPDs.  Therefore, the best practice for extension is to group all <termRef term="data component">data components</termRef> designed to be reused into one extension schema document or document set, and group IEPD-specific <termRef term="data component">data components</termRef> into another.  Then the reusable extension components can be more easily redeployed in other IEPDs as needed.</p> 

	<p>Extension schema documents generally contain new <termRef>data component</termRef> declarations that may (though not necessarily) be derived from or reference existing NIEM <termRef term="data component">data components</termRef>.  This being the case, reference schema documents do not exist for new <termRef term="data component">data components</termRef> found within extension schema documents.  Therefore, extension schema documents must satisfy the more rigorous documentation requirements of reference schema documents.  Per the <ref idref="NIEM-NDR"/>, the definition or declaration of each new <termRef>data component</termRef> in an extension schema document must include an <qName>xs:annotation</qName> element that provides its semantics and NIEM-specific relationships.</p>

</section>



<section id="External-Schemas"><title>External Schema Documents</title>
	...
	<definition term="external schema document">
	  ...
	  <definition term="external adapter type">
		  <p>As defined by <ref idref="NIEM-NDR"/>:</p>
		  <blockquote>
			  <p>A NIEM-conformant type that adapts external components for use within NIEM.  An external adapter type creates a new class of object that embodies a single concept composed of external components.  External adapter types are defined in NIEM-conformant schemas.</p>
		  </blockquote>
 	 </definition>
...
<section id="Constraint-Schemas"><title>Constraint Schema Document Sets</title>

	<definition term="constraint schema document set">
		<p>A set of related constraint schema documents that work together; for example, a constraint schema document set could be built by adding constraints to a schema document subset.</p>
	</definition>

	<p>A <termRef>constraint schema document set</termRef> is an <termRef>XML schema document</termRef> set that is used to express business rules for a class of <termRef term="instance XML document">instance XML documents</termRef>; it is not intended to provide definitions for the semantics of the individual components it contains.  Instead, a constraint schema document set uses the XML Schema Definition Language to add constraints to components defined or declared by other schema documents, usually a <termRef>schema document subset</termRef>; but they can be applied to <termRef term="extension schema document">extension schema documents</termRef> as well.</p>
...
<section id="IEPD-Documentation-Artifacts"><title>IEPD Documentation Artifacts</title>
	
	<p>XML schema documents (and the schemas that result from them) are the essence of a NIEM <termRef term="information exchange package documentation">IEPD</termRef>.  However, a variety of documentation files may be incorporated into a NIEM <termRef term="information exchange package documentation">IEPD</termRef>.  One particular documentation file is required in every IEPD:  <code>iepd-catalog.xml</code>, the IEPD catalog document.  This file contains basic metadata, relationship and lineage data, <termRef>conformance target</termRef> specifications, and validation information.</p>

	<p>A <termRef>readme artifact</termRef> (formerly known as a <em>master document</em>) is mandatory for IEPDs.  IEPDs are often built by different developers, and may be registered into a repository for reuse by many other users, developers, and implementers; therefore, a minimal form of documentation is absolutely necessary.  An <termRef term="information exchange package documentation">IEPD</termRef> readme file is the primary source and starting point for human readable documentation, and should reference (and describe) any other separate documentation artifacts.  This requirement ensures that baseline documentation is consistently rooted in a clearly visible artifact within each <termRef term="information exchange package documentation">IEPD</termRef>.</p>
...
<!--<section id="mpdClass"><title>MPD Class (<qName>c:mpdClassURIList</qName>)</title>-->
	<section id="iepdConformanceTargetIdentifier"><title>IEPD Conformance Target Identifier (<qName>c:iepdConformanceTargetIdentifierURIList</qName>)</title>
		
		<p>An <termRef>IEPD conformance target identifier</termRef> is a <termRef>conformance target identifier</termRef> to which the given <termRef term="information exchange package documentation">IEPD</termRef> claims to conform.  The IEPD catalog <qName>c:iepdConformanceTargetIdentifierURIList</qName> attribute declares a list of <termRef term="conformance target identifier">conformance target identifiers</termRef>, identifying the <termRef term="conformance target">conformance targets</termRef> to which the IEPD claims to conform.</p> <!--The following rule establishes the <termRef term="MPD class">class</termRef> of an <termRef term="model package description">MPD</termRef>:</p>
	...
	<p>It should be clear that an <termRef term="model package description">MPD</termRef> that is an <termRef term="information exchange package documentation">IEPD</termRef> should have a value for its <qName>c:mpdClassURIList</qName> attribute that contains both <termRef term="conformance target identifier">conformance target identifiers</termRef> above.  In the future, additional conformance target identifiers will be assigned by other appropriate NIEM specifications that specialize MPDs (for example, releases, domain updates, etc.).</p>-->

	<p>The <qName>c:iepdConformanceTargetIdentifierURIList</qName> attribute is an XML list type that may declare that an IEPD conforms to multiple conformance targets.  An IEPD developer can establish a new IEPD <termRef>conformance target</termRef> identifier in addition to those provided by this and other NIEM specifications.  The identifier represents the new conformance target which should be associated with one or more rules or constraints to which an IEPD must conform if it is assigned that identifier.</p>
 ...
 "MACRO_MPD_conformance_target_identifier
  ...
  http://example.org/niem-iepd/1.0/#abc-org"</pre>
	...
	<figure id="abc"><title>IEPD catalog <qName>c:iepdConformanceTargetIdentifierURIList</qName> attribute for organization ABC.</title>
...
<section id="IEPD-Relationships">
	...
	<p>As was described in <ref idref="externalURI"/>, the IEPD catalog also enables an author to record more fine-grained ancestry between IEPDs using the <qName>c:externalURI</qName> attribute.  This attribute records an explicit equivalence relationship between artifacts reused across IEPDs.</p>

	<p>Note that a <qName>c:resourceURI</qName> attribute is used to identify a remote resource that is only related to the IEPD whose catalog declares it.  The resource is not required for validation.  Therefore, the <termRef term="information exchange package documentation">IEPD</termRef> is not required to contain this resource.  As in the case of <qName>c:externalURI</qName>, the value of a <qName>c:resourceURI</qName> should be correct (i.e., a resource with that URI should exist).  However, in this case, existence verification is considered outside the scope of this specification.</p>
...
<section id="URI-Resolution-Ref-Elements">
	...
	<p>A NIEM element can indirectly reference its content rather than carry or encapsulate it.  A NIEM
	element with simple content derived from type <qName>xs:anyURI</qName> may appear in an instance XML
	document as a reference element, in which case, rather than locally containing a URI as simple content,
	it will instead refer to another element that contains a URI. Under some circumstances, this might
	impact URI resolution described in <ref idref="r-resolve-uri"/>.</p>
	...
	<definition term="reference element">
		<p>A reference element is an element information item that has an attribute <qName>structures:ref</qName>.  A reference element refers to its value by reference, instead of carrying it as content.</p>
	</definition>
...
<section id="XML-Catalog-URI"><title>XML Catalog URI</title>
	...
	<ruleSection><title>XML Catalog <code>uri</code> Value Resolves to Resource</title>
		<rule id="xc1" applicability="XML-catalog" class="Constraint">
			<p>Within an <termRef>XML catalog document</termRef>, the value of a <code>uri</code> attribute owned by a <qName>er:uri</qName> element MUST <termRef term="resolve URI">resolve</termRef> to a resource.</p>
			</rule>
	</ruleSection>
	...
	<ruleSection><title>XML Catalog <code>uri</code> Value Resolves to Resource with Correct Target Namespace</title>
		<rule id="xc2" applicability="XML-catalog" class="Constraint">
			<p>Within an <termRef>XML catalog document</termRef>, given an <termRef>XML schema document</termRef> resolved by the value of a <code>uri</code> attribute owned by a <qName>er:uri</qName> element, the <termRef>XML schema document</termRef> target namespace MUST equal the value of the <code>name</code> (a namespace string) attribute owned by the <qName>er:uri</qName> element.</p> 
		</rule>
	</ruleSection>
...
<section id="URI-Summary"><title>Summary of IEPD URIs</title>
		...
		<table id="uri-summary"><title>Summary of IEPD URI attributes</title>
			...
			<tr>
			    <td><qName>c:resourceURI</qName></td>
			    <td>&lt;URI&gt;</td>
			</tr>
		</table>
...
<!--<section id="Change-Log-Releases-Core-Domain-Updates">
	...
	<p>New versions of reference schema document sets such as NIEM releases and domain updates can have significant impacts on future IEPDs.  Developers must understand in detail how changes will affect their <termRef term="information exchange package documentation">IEPD</termRef> products and the tools used to build them.  To work effectively, tools for domain content development, impact analysis, migration between releases, etc. must be able to digest formal change logs.  A formal change log is also essential to efficiently process and integrate new and changed content into NIEM for new releases, and to simultaneously maintain multiple versions of NIEM for users.  All of the foregoing reasons dictate that a NIEM reference schema document set requires a normative change log.</p>
...
<!--<section id="Change-Log-IEPD"><title>Change Log for IEPDs</title>-->
	...
	<p>The <termRef>change log</termRef> for the first version release of an IEPD simply contains its release date.</p>
...
<section id="ReadMe-Artifact"><title>ReadMe Artifact</title>
	...
	<p>The primary purposes of the <termRef>readme artifact</termRef> include:</p>
		<ul>
			<li><p>To help facilitate understanding and reuse of IEPDs.</p></li>
			<li><p>To ensure that fundamental and detailed business-level information about an <termRef term="information exchange package documentation">IEPD</termRef> are documented for human understanding.</p></li>
			<li><p>To ensure the <termRef term="information exchange package documentation">IEPD</termRef> author has considered and conveys such fundamental information.</p></li>
...
<section id="Readme-Content"><title>Readme Content</title>

	<p>This section is non-normative.</p>

	<p>This section is neither a cookbook nor a normative specification for a <termRef>readme artifact</termRef>.  It simply suggests typical topics that a readme artifact should or might address, and provides some non-normative guidance.</p>
	...
	<li><p>Implementation guidance (particularly important for a complex <termRef term="information exchange package documentation">IEPD</termRef> with multiple subsets or IEP root elements)</p></li>
	...
	<li><p>If an <termRef term="information exchange package documentation">IEPD</termRef> employs multiple subsets:</p>
	<ul>
		<li><p>When, where, and how these are used</p></li>
		<li><p>How these are coordinated in the implementation</p></li>
		<li><p>Caveats regarding duplicate <termRef term="data component">data components</termRef> (which can occur with multiple subsets)</p></li>
	</ul></li>
	<li><p>If an <termRef term="information exchange package documentation">IEPD</termRef> employs multiple IEP conformance targets:</p>
	<ul>
...
<section id="XML-Catalogs"><title>XML Catalogs</title>

	<definition term="XML catalog document">
		<p>As defined by <ref idref="OASIS-XML-Catalogs"/>, which states:</p>
		<blockquote><p>An entity catalog.</p></blockquote>

		<p>An <termRef>XML catalog document</termRef> MUST conform to all the rules in this specification for the conformance target <termRef>XML catalog document</termRef>.</p>

		<p>Rules specifying this conformance target use the applicability code <q>XML-catalog</q>.</p>
	</definition>
	...
	<p>Note that schema assembly or <termRef>XML catalog document</termRef> design from non-conformant <termRef term="external schema document">external schema documents</termRef> that may contain <qName>xs:include</qName> statements can be problematic.</p>

	<p>In order to support <termRef>XML schema assembly</termRef> for the purpose of <termRef>XML schema validation</termRef>, the namespace of each XML schema document used within an <termRef term="information exchange package documentation">IEPD</termRef> should <termRef term="resolve URI">resolve</termRef> to a locally-unique artifact.  A correctly constructed <termRef>XML catalog document</termRef> can guarantee this.</p>

	<p>According to <ref idref="OASIS-XML-Catalogs"/>, <qName>er:nextCatalog</qName> elements may be used within <termRef term="XML catalog document">XML catalog documents</termRef> to connect them and control the parsing sequence.  <ref idref="Defining-Information-Exchange-Packages"/> discusses more about using <termRef term="XML catalog document">XML catalog documents</termRef> within IEPDs.</p>
...
<section id="Defining-Information-Exchange-Packages">
	...
	<ruleSection>
		...
		<ol>
			...
			<li><p>a locally unique NCName (i.e., a non-colonized name, as defined by <ref idref="W3-XML-Schema-Datatypes"/>, <a href="http://www.w3.org/TR/2004/REC-xmlschema-2-20041028/datatypes.html#NCName"><char name="sect"/>3.3.7, <q>NCName</q></a>).</p></li>
		</ol>
		</rule>
...
<ruleSection>
	...
	<p>The following subsections detail the concepts, artifacts, and procedures for declaring and identifying <termRef term="IEP conformance target">IEP conformance targets</termRef> in IEPDs.</p>
...
<section id="Validity-Context-Constraints"><title>Validity Context and Constraints</title>
	...
	<definition term="validity constraint context">
			<p>A set of information items that establishes the applicability of certain validity constraints.</p>
	</definition>

	<p>Validity constraint context can refer to multiple information items (e.g., XML elements, attributes, etc.) within an IEP.  Also, note that <termRef>validity constraint context</termRef> can evaluate to no information items (e.g., in XPath, <q><code>()</code></q>, for which <q><code>empty-sequence()</code></q> is true).  In this cases, the validity constraints (within the in scope validity constraint context) will not fire and the test passes.</p>

	<p>The following subsections explain in more detail the purpose and context for validity constraints that can be declared in <qName>c:IEPConformanceTarget</qName>.</p>

</section>



<section id="Declaring-Validity-Constraints"><title>Declaring Validity Constraints</title>
...
<section id="ValidityConstraint"><title>c:ValidityConstraint</title>
...
<section id="ValidityContext"><title>c:ValidityContext</title>
...
<section id="HasDocumentElement"><title>c:HasDocumentElement</title>

	<p><qName>c:HasDocumentElement</qName> is a validity constraint that identifies all intended XML document elements for an IEP conformance target, and it is directly substitutable for <qName>c:ValidityConstraintWithContext</qName>.  This constraint ensures that an IEP artifact is rooted by one XML document element that is a member of the list of elements in its <qName>c:qualifiedNameList</qName> attribute.  This is a common validity constraint employed by simple IEPDs that declare one or more intended XML document elements.</p>

	<p>Note that <termRef>validity constraint context</termRef> of <qName>c:HasDocumentElement</qName> is always on the IEP<char name="rsquo"/>s <em>document information item</em> as defined in <ref idref="W3-XML-InfoSet"/>, <a href="http://www.w3.org/TR/2004/REC-xml-infoset-20040204/#infoitem.document"> <char name="sect"/>2.1, <q>The Document Information Item</q></a>.  This is because it can only declare XML document elements.  So, if an IEP defines a payload that may be included in some XML envelope, then <qName>c:HasDocumentElement</qName> should not be used.  Instead, use <qName>c:ValidityContext</qName> with another specific validity constraint and <qName>c:xPathText</qName> to explicitly declare <termRef>validity constraint context</termRef>.</p>

	<p>When employing <qName>c:HasDocumentElement</qName> the following rule applies:</p>
...
<section id="ValidToXPath"><title>c:ValidToXPath</title>

	<p><qName>c:ValidToXPath</qName> is a specific validity constraint whose purpose is to ensure that a condition is satisfied within an IEP.  The condition is defined by an XPath expression contained in the <qName>c:xPathText</qName> attribute.  If the XPath expression applied to a target instance <termRef>XML document</termRef> returns a Boolean value of TRUE, then the condition is satisfied by that XML document.</p>

	<p>This validity constraint is useful for a variety of purposes.  For example, an <termRef term="information exchange package documentation">IEPD</termRef> author may require that a given <qName>c:IEPConformanceTarget</qName> must contain a particular element with a particular attribute whose value is an integer greater than some required minimum.  An XPath expression can validate this.</p>

	<p><qName>c:ValidToXPath</qName> can also employ a simple XPath expression to validate that an IEP is rooted with an intended XML document element.  However, other validity constraints can do this as well; the <termRef term="information exchange package documentation">IEPD</termRef> author may choose the constraint representation.</p>

	<p>Note that if <qName>c:ValidToXPath</qName> is used (substituted) within <qName>c:ValidityContext</qName> there will be two XPath expressions <char name="mdash"/> the expression within <qName>c:ValidToXPath</qName> is the condition to validate, the other is the context (where the condition will be validated).  For example, the context provided by <qName>c:ValidityContext</qName> might be <code>//my:speedingTicket</code>, while the <qName>c:ValidToXPath</qName> might require that a test for <code>exists(nc:DriverPerson)</code> be true.</p>

	<p>This specific validity constraint as well as those that follow below can either be substituted for the <qName>c:ValidityConstraint</qName> or used within the <qName>c:ValidityContext</qName> element (i.e., substituted for its <qName>c:ValidityConstraint</qName> child).</p>

	<p>Note that if <qName>c:ValidToXPath</qName> is substituted for <qName>c:ValidityConstraint</qName> within the <qName>c:ValidityContext</qName> element, then the explicit context, the <qName>c:xPathText</qName> value, can imply that multiple items must be checked and each must return "true" in order for an IEP to pass the <qName>c:ValidToXPath</qName> constraint.</p>
...
<section id="XMLSchemaValid"><title>c:XMLSchemaValid</title>
...
<section id="SchematronValid"><title>c:SchematronValid</title>

	<p><qName>c:SchematronValid</qName> is similar to <qName>c:XMLSchemaValid</qName>, but uses a <qName>c:SchematronSchema</qName> element to identify the Schematron rule file that applies to the IEP.</p>
</section>


<section id="RelaxNGValid"><title>c:RelaxNGValid</title>

	<p><qName>c:RelaxNGValid</qName> is similar to the previous two validity constraints, but uses a <qName>c:RelaxNGSchema</qName> element to identify the RelaxNG schema file to which the IEP must validate.</p>
</section>


<section id="ConformsToConformanceTarget"><title>c:ConformsToConformanceTarget</title>
...
<section id="ConformsToRule"><title>c:ConformsToRule</title>

	<p>Sometimes it is not possible to formally declare an executable validity constraint.  For example, we can mandate that a <termRef>data component</termRef> definition must be present, must be in English, and must follow <ref idref="ISO-11179-4"/>.  Validating that text is present is easy, and validating that it is in English is more difficult, but validating that it obeys <ref idref="ISO-11179-4"/> is essentially intractable.  Thus, <qName>c:ConformsToRule</qName> provides an <termRef term="information exchange package documentation">IEPD</termRef> author with English text representation as an alternative when it is not possible or not easy to define more formal validation rules or validity constraints.</p>
...
<section id="IEP-Samples"><title>IEP Sample Instance XML Documents</title>
	...
	<ul>
		<li><p>Help an <termRef term="information exchange package documentation">IEPD</termRef> implementer to understand the original intent of the <termRef term="information exchange package documentation">IEPD</termRef> author.</p></li>
		<li><p>Can be used by an implementer as a data point for validation of IEP conformance targets.</p></li>
		<li><p>can indicate or imply <termRef term="information exchange package documentation">IEPD</termRef> quality.</p></li>
	</ul>

	<p>For these reasons, the following rule applies to an <termRef term="information exchange package documentation">IEPD</termRef>:</p>
	...
	<p>The rule above requires that each declared <termRef>IEP conformance target</termRef> be covered (exemplified or correctly demonstrated) by at least one IEP sample instance XML document.  This does not necessarily mandate a different IEP sample for each <termRef>IEP conformance target</termRef>.  It may be possible, and is therefore acceptable, for a given IEP sample to serve as an example of one or more <termRef term="IEP conformance target">IEP conformance targets</termRef>.</p>
	
	<p>The purpose of this rule is not to provide a test for all possible IEP permutations given the schema definitions and validity constraint declarations; rather, it is to encourage <termRef term="information exchange package documentation">IEPD</termRef> authors to test their own designs, and to provide implementers with examples for additional understanding, guidance, and testing.  To the extent possible, <termRef term="information exchange package documentation">IEPD</termRef> authors should strive to include sample IEPs that (1) capture real world business cases of data exchanges, and (2) exercise as many <termRef term="data component">data components</termRef> and validity constraints as possible.  Where it makes sense, an <termRef term="information exchange package documentation">IEPD</termRef> author should strive to provide enough sample IEPs to exercise all the XML document elements (or payload root elements).  If a single IEP cannot provide enough example coverage, an author may include multiple IEPs (but is not required to do so).</p>

	<p>Each sample IEP usually illustrates a single view of the data based on a chosen set of conditions.  Other views based on different conditions likely exist.  An implementer will still need to review the <termRef term="information exchange package documentation">IEPD</termRef> documentation to ensure understanding of all potential conditions.  Therefore, as appropriate, the author should not rely exclusively on sample IEPs to convey implementation understanding, since they will probably not account for all possible permutations.</p>
...
<section id="Conformance-Assertion"><title>Conformance Assertion</title>
	...
	<p>A <termRef>conformance assertion</termRef> provides information to increase the level of confidence that an <termRef term="information exchange package documentation">IEPD</termRef> was checked for NIEM conformance and quality.  It does NOT constitute a guarantee or contract.  In fact, a conformance assertion can be self-asserted.</p>
...
<section id="Wantlists"><title>NIEM Wantlist</title>

	<p>A NIEM schema document subset is often associated with a NIEM <em>wantlist</em>.  A <em>wantlist</em> is an abbreviated XML representation of a NIEM schema document subset, and identifies only the <termRef term="data component">data components</termRef> a user selected (as requirements) to build a schema document subset.  To reconstruct the complete schema document subset there are usually a number of additional <termRef term="data component">data components</termRef> that the user selections depend upon.  These must be computed from the appropriate NIEM reference model and added to reconstruct the complete schema document subset.  For example, a user may select <qName>nc:Person</qName> for the subset.  In this case, the wantlist will only contain that component, but the associated full subset must contain both <qName>nc:Person</qName> and <qName>nc:PersonType</qName>.  A software tool that understands how to process NIEM wantlists and schema document subsets (such as the NIEM Schema Subset Generator Tool <ref idref="NIEM-SSGT"/>) can rebuild an accurate schema document subset from a wantlist (and the reverse).</p>
...
<section id="Business-Rules"><title>Business Rules</title>
	...
	<p><termRef term="business rules">Business rules</termRef> may be represented as informal English statements, or as formally coded machine-readable and process-able statements.  For example, an <termRef term="information exchange package documentation">IEPD</termRef> may use a <termRef>Schematron schema</termRef> <ref idref="ISO-Schematron"/> or any other formal representation for <termRef>business rules</termRef>.</p>
...
<section id="Artifact-Sets"><title>Artifact Sets</title>
	...
	<figure>
		<title>Simple <qName>c:FileSet</qName> form for a directory-associated artifact set.</title>
		<pre>
...
<section id="IEPD-File-Name-Syntax"><title>IEPD File Name Syntax</title>
...
<ruleSection>
	...
	<rule id="r-iepd-name-syntax" applicability="IEPD" class="Constraint">
		<p>The file name of an <termRef term="information exchange package documentation">IEPD</termRef> <termRef>ZIP file</termRef> (<code>iepd-filename</code>) SHOULD adhere to the syntax defined by the regular expression:</p>
	<pre>
...
<section id="IEPD-Completeness"><title>IEPD Completeness</title>

	<p>Since an <termRef term="information exchange package documentation">IEPD</termRef> defines an information exchange and is often implemented by persons other than the original author, it is important to ensure that they are relatively complete and provide all artifacts needed to use the <termRef term="information exchange package documentation">IEPD</termRef>.</p>
...
<section id="IEPD-Artifacts" isAppendix="true"><title>Common IEPD Artifacts</title>
	...
	<p>Notes:</p>
	<ul>
		<!--<li><p>(MPD) in artifact name indicates a required artifact for all <termRef term="model package description">MPDs</termRef>.</p></li>-->
		<li><p>(IEPD) in artifact name indicates a required artifact for an <termRef term="information exchange package documentation">IEPD</termRef>.</p></li>
		<li><p>(ref) indicates name is hyperlinked to a defined term in the specification.</p></li>
...
<table id="iepd-artifacts">
	...
	<tr><td>(IEPD) <termRef>IEPD catalog document</termRef></td><td>iepd-catalog.xml</td><td>(ref)</td></tr>
	<tr><td>(IEPD) <termRef>change log</termRef></td><td>*.*</td><td>(ref)</td></tr>
	...
	<tr><td>IEP sample <termRef>XML document</termRef></td><td>*.xml</td><td>an XML document instance that exemplifies an <termRef>IEP conformance target</termRef> defined by a <qName>c:IEPConformanceTarget</qName> (ref)</td></tr>
	<tr><td>Schematron schema document</td><td>*.sch</td><td>a business rule schema in <ref idref="ISO-Schematron"/> format</td></tr>
	<tr><td>RelaxNG schema document</td><td>*.rng</td><td>a business rule schema in <ref idref="ISO-RelaxNG"/> format</td></tr>
...
<section id="Conformance-Assertion-Example" isAppendix="true">
	...
	<p>A NIEM conformance assertion is a required artifact for an <termRef term="information exchange package documentation">IEPD</termRef>.  The following is a simple example of a conformance assertion (in this case, a self assertion by the author, but with assistance from colleagues.  The concept is to provide implementers with some information that indicates how well an <termRef term="information exchange package documentation">IEPD</termRef> has been checked for quality and conformance with respect to XML Schema and NIEM.  The assertion can be as simple as the <em>assertion</em> clause.  However, clearly the more detail that is provided, the stronger the case for conformance and quality will be.</p>
...
<section id="Guidance-IEPD-Directories" isAppendix="true">
	...
	<p>NIEM releases (and their associated core supplements) and domain updates generally follow a consistent directory organization.  When employing a release, its supplements, and updates within IEPDs whether as-is or as subsets, developers are encouraged to maintain their original directory structures.  However, aside from applicable rules previously stated in the preceding sections, there are no normative rules for organizing directories within IEPDs.</p> 

	<p>Guidance for directory structuring may be useful to authors, especially in the case of a relatively simple <termRef term="information exchange package documentation">IEPD</termRef> with a single schema document subset, and a few extension and external schema documents.  The following are common non-normative practices for <termRef term="information exchange package documentation">IEPD</termRef> directories:</p>
...
<ol>
	...
	<li><p>Create the following directories within the <termRef>IEPD root directory</termRef>:</p>
		<ul>
			<li><p><code>base-xsd</code> <char name="mdash"/> will contain the NIEM subset and its associated extension, external, and custom NIEM schema documents.  These are the NIEM XML schema documents used to validate conformance of an instance XML document.  Subdirectories under <code>base-xsd</code> may include:</p>
				<ul>
					<li><p><code>niem</code> <char name="mdash"/> a NIEM schema subset organized as the Schema Subset Generation Tool (SSGT) generates it (including <code>wantlist.xml</code> and <code>xml-catalog.xml</code> artifacts).</p></li>
					<li><p><code>extension</code> <char name="mdash"/> for NIEM extension schema documents.</p></li>
					<li><p><code>external</code> <char name="mdash"/> for non-NIEM standards used by the <termRef term="information exchange package documentation">IEPD</termRef>.</p></li>
	...
	<li><p>If using specialized <termRef term="XML schema document">XML schema documents</termRef> for optimized EXI serialization, create an <code>exi-xsd</code> subdirectory for them.</p></li>
	...
	<li><p>Maintain an <code>xml-catalog.xml</code> closest to the XML schema document set it relates to.  Use <code>er:nextCatalog</code> elements as needed to help maintain this proximity).</p></li>
...
<section id="Acronyms-and-Abbreviations" isAppendix="true"><title>Acronyms and Abbreviations</title>
...
<table id="acronyms-abbreviations">
	...
	<tr><td>NCName</td><td>Non-colonized Name (in XML Schema: unprefixed and no colon character)</td></tr>
	...
	<tr><td>TDF  </td><td>Trusted Data Format</td></tr>
...
<reference id="LEXS" label="Logical Entity Exchange Specification">
	<p><em>Logical Entity Exchange Specification</em>, Version 4.0, 27 July 2011.	Available from <link>http://130.207.211.107/content/downloads</link>.</p>
</reference>
...
<reference id="NIEM-CTAS" label="NIEM Conformance Targets Attribute Specification 3.0">
	<p><em>NIEM Conformance Targets Attribute Specification</em>, Version 3.0, NIEM Technical Architecture Committee (NTAC), 31 July 2014.  Available from <link>http://reference.niem.gov/niem/specification/conformance-targets-attribute/3.0/</link>.</p>
</reference>
...
<section id="Revision-History" isAppendix="true">
	...
	<ol>
		<li><p>Changed term <q>core update</q> to <q><a href="http://reference.niem.gov/niem/specification/high-level-version-architecture/3.0/high-level-version-architecture-3.0.html#definition_core_supplement">core supplement</a></q>.</p></li>
		<li><p>Removed references to outdated <em>NIEM User Guide Volume 1</em>.</p></li>
		<li><p>Relaxed and clarified MPD ZIP file format requirement.  Allows an MPD to live as a file collection in a repository, such as a Git repository, and still use ZIP file format for transit, upload, download, bulk storage, or whenever convenient to maintain a small electronic footprint.</p></li>
		<li><p>Changed a <a href="http://reference.niem.gov/niem/specification/high-level-version-architecture/3.0/high-level-version-architecture-3.0.html#definition_core_supplement">core supplement</a> from a type of MPD to a special <a href="http://reference.niem.gov/niem/specification/high-level-version-architecture/3.0/high-level-version-architecture-3.0.html#definition_NIEM_release">release</a>.  The MPD specification now defines only four MPD classes:  <a href="http://reference.niem.gov/niem/specification/high-level-version-architecture/3.0/high-level-version-architecture-3.0.html#definition_NIEM_release">release</a>, <a href="http://reference.niem.gov/niem/specification/high-level-version-architecture/3.0/high-level-version-architecture-3.0.html#definition_domain_update">domain update</a>, <termRef term="information exchange package documentation">IEPD</termRef>, and Enterprise Information Exchange Model (EIEM) (described in <ref idref="NIEM-BIEC"/>).</p></li>
		<li><p>Modified <ref idref="Scope"/> to explain that this specification is applicable to all MPDs in general, but is specifically focused on <termRef term="information exchange package documentation">IEPD</termRef>.</p></li>
	</ol>
</section>
